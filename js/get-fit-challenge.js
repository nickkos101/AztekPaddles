...
function calculateTotalPaddleLength() {
	...
	//ADD TWO GLOBAL TO PADDLE LENGTH
...
function validation() {
	 ...
	 function change_results_page() {
	 	...
	 	jQuery('.complete .bladeType').val(calculateBladeTypes());
	 	jQuery('.complete .bladeSize').val(calculateBladeSize());
	 	jQuery('.complete .paddleLength').val(calculateTotalPaddleLength());
	 	jQuery('.complete .flexRating').val(calculateShaftFlex());
	 	jQuery('.complete .handleSize').val(calculateHandleSize());
	 	...
	 	if (calculateBladeTypes() == 'Race and Surf') {
	 		jQuery('.complete .bladeType').val('Surf');
	 		jQuery('.complete-paddle-img').attr('src',surfPaddleImgURL);
...
function add_to_cart(isCompletePaddle) {
	...
	var pbladeType = jQuery('.c-paddle-add').parent().parent().find('.bladeType').val();
	var pbladeSize = jQuery('.c-paddle-add').parent().parent().find('.bladeSize').val();

	console.log(pbladeType);

	var ptotalpaddleLength = jQuery('.c-paddle-add').parent().parent().find('.paddleLength').val();

	var pshaftFlex = jQuery('.c-paddle-add').parent().parent().find('.flexRating').val();

	var phandleSize = jQuery('.c-paddle-add').parent().parent().find('.handleSize').val();
...
function ShowLoader() {
	jQuery('.load').fadeToggle();
	jQuery('.lightbox-bg').fadeToggle();
}

ShowLoader();

jQuery('.load .checkbox').click(function(){
	jQuery('.load .checkbox').removeClass('active');
	jQuery(this).toggleClass('active');
	if (jQuery(this).hasClass('fit-one')) {
		ActiveSlot = 'fit_profile_one';
	}
	else if (jQuery(this).hasClass('fit-two')) {
		ActiveSlot = 'fit_profile_two';
	}
	else if (jQuery(this).hasClass('fit-three')) {
		ActiveSlot = 'fit_profile_three';
	}
	else {
		ActiveSlot = 'fit_profile_one';
	}
...
function loadFromDataBase(slot) {
	var url = 'http://'+window.location.hostname+'?load_fit='+slot;
...
//Save the Fit Profiles and then clear lightboxes
jQuery('.load-it').click(function(){
	loadFromDataBase(ActiveSlot);
	clearLightBoxes();
});
...
jQuery('.slots .checkbox').click(function(){
	jQuery('.slots .checkbox').removeClass('active');
	jQuery(this).toggleClass('active');
	if (jQuery(this).hasClass('fit-one')) {
		ActiveSlot = 'fit_profile_one';
	}
...
function saveSlot(slot) {
	...
	jQuery.get( url, { save_fit: ActiveSlot, fit_data: allVars}, function( data ) {
		if (data != '') {
